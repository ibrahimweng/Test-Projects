...
only thing lost is the up and down motion.

## Accessibility

The tabs are a standard tab list. You can move between them with the left and
right arrow keys. Press Home or End to go to the first or last tab. The card
area is marked as a live region, so a screen reader will read the new text when
it changes.

If a visitor has asked their system to reduce motion, the up and down motion is
turned off and all the transitions are instant.
